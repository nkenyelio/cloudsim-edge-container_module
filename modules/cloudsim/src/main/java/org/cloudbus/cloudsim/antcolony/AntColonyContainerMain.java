...
public class AntColonyContainerMain {
	...
	private static List<? extends ContainerCloudlet> cloudletList;
	...
	private static List<? extends ContainerVm> vmlist;
	...
	private static List<Container> containerList1;
	...
	private static List<ContainerVm> createVM(int userId, int vms) {

		//Creates a container to store VMs. This list is passed to the broker later
		LinkedList<ContainerVm> list = new LinkedList<ContainerVm>();
		...
		/** The peList. */
	    LinkedList<ContainerPe> peList = new LinkedList<ContainerPe>();
	    //ContainerScheduler containerScheduler = new ContainerScheduler(peList); 
		//create VMs
		ContainerVm[] vm = new ContainerVm[vms];

		for(int i=0;i<vms;i++){
			mips = 500 + r.nextInt(500);
			vm[i] = new ContainerVm(i, userId, mips, ram, bw, size, vmm, new ContainerScheduler(peList), new ContainerRamProvisioner(ram), new ContainerBwProvisioner(bw), peList);
	...
	private static List<? extends ContainerCloudlet> createCloudlet(int userId, int cloudlets){
		// Creates a container to store Cloudlets
		LinkedList<ContainerCloudlet> list = new LinkedList<ContainerCloudlet>();
		...
		ContainerCloudlet[] cloudlet = new ContainerCloudlet[cloudlets];

		for(int i=0;i<cloudlets;i++){
			length = 100 + r1.nextInt(900);
			// fileSize = 100 + r2.nextInt(200);
			cloudlet[i] = new ContainerCloudlet(i, length, pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel);
...
*/
	public static void main(String[] args) {
		...
		try {
			...
			for (int k=1;k<=10;k++){
				...
				DatacenterbrokerAntOpt broker = createBroker();
				...
				dataset.addValue((Number)(max-min),"AntColony Algo",String.valueOf(100*k));
			...
			AWTChart.implement("Response Time Vs. Cloudlet","No. of cloudlets","Total response time",dataset);
	...
	//We strongly encourage users to develop their own broker policies, to submit vms and cloudlets according
		...
		private static DatacenterbrokerAntOpt createBroker(){

			DatacenterbrokerAntOpt broker = null;
			try {
				//int m, double Q, double alpha, double beta, double gamma, double rho
				broker = new DatacenterbrokerAntOpt("Broker",80,37,1,2,1,4,0.05);
			} catch (Exception e) {
				e.printStackTrace();
				return null;
			}
