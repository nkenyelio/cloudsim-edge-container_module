...
public class AntColonyOptDefault {protected double Q;
...
public Map<Integer,Integer> implement(List<Cloudlet> clList,List<Vm> vm_list,int tmax) throws FileNotFoundException{
	int tasks = clList.size();
	int vms = vm_list.size();
	...
	for(int i=0;i<tasks;i++){
		...
		for (int j=0; j<vms ; j++) {
			double t = getExecutionTime(vm_list.get(j),clList.get(i));
			x.put(j,t);
		}
		execTimes.put(i,x);
	}
...
public AntColonyOptDefault(int m, double Q, double alpha, double beta, double gamma, double rho){
	this.m = m;
	this.Q = Q;
	this.alpha = alpha;
	this.beta = beta;
...
protected double getExecutionTime(Vm containerVm,Cloudlet cloudlet){
	return (cloudlet.getCloudletLength()/(containerVm.getNumberOfPes()*containerVm.getMips()));
}
}
